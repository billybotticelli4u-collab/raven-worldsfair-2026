...
test('writer and reader use platform precedence; stale generated output is not recycled', () => {
 const temp=mkdtempSync(path.join(tmpdir(),'raven-writer-'));
 try {
  mkdirSync(path.join(temp,'src/lib'),{recursive:true}); mkdirSync(path.join(temp,'public'));
  writeFileSync(path.join(temp,'package.json'),' {"type":"module"}');
  for(const relative of ['src/write-build-info.js','src/lib/buildIdentity.js'])
   writeFileSync(path.join(temp,relative),readFileSync(fileURLToPath(new URL('../'+relative,import.meta.url))));
  for(const file of ['app.js','index.html','styles.css']) writeFileSync(path.join(temp,'public',file),file);
  writeFileSync(path.join(temp,'generated-build-info.json'),JSON.stringify({commit:'c'.repeat(40)}));
  const env={...process.env,VERCEL_GIT_COMMIT_SHA:'b'.repeat(40),WORLDSFAIR_BUILD_COMMIT:'a'.repeat(40)};
  execFileSync(process.execPath,['src/write-build-info.js'],{cwd:temp,env});
  const generated=JSON.parse(readFileSync(path.join(temp,'generated-build-info.json')));
  assert.equal(generated.commit,'b'.repeat(40));
  assert.equal(generated.source,'platform_asserted');
  assert.equal(readIdentity(temp,{env}).fairBuildCommit,generated.commit);
  assert.equal(generated.identityClaims.some(c=>c.commit==='c'.repeat(40)),false);
